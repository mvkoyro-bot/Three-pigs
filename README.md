[README.md](https://github.com/user-attachments/files/26513725/README.md)
# Три поросенка

## Диаграммы

| Диаграмма | Описание | Файл |
|-----------|----------|------|
| Use Case | Варианты использования системы | [use_case (1).md](use_case.md) |
| Class | Диаграмма классов (ООП структура) | [class.md](class.md) |
| Activity | Алгоритм работы системы | [activity.md](activity.md) |
| Sequence | Пошаговое взаимодействие объектов | [sequence.md](sequence.md) |
| State | Жизненный цикл Золотого Яйца | [state.md](state.md) |
